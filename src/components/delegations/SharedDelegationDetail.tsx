  ...
  return (
    <div className="mx-auto max-w-5xl space-y-8 pb-10 duration-500 animate-in fade-in">
      ...
      <div ref={tabRef} className="flex gap-4 border-b border-slate-200">
        ...
        <TabButton active={activeTab === "members"} onClick={() => setActiveTab("members")} label="THÀNH VIÊN ĐOÀN" icon={<Users size={14} />} />
        ...
        <div className="space-y-8 md:col-span-2">
                  ...
                  <div className="mt-10 grid grid-cols-2 gap-8 border-t border-slate-100 pt-8 sm:grid-cols-3">
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-sm font-bold text-emerald-600">
                        {d.investment_potential ? new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(d.investment_potential) : "N/A"}
                      </p>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    {d.contacts?.length ? d.contacts.map((c, idx: number) => (
                      <div key={idx} className="space-y-3 rounded-lg border border-slate-50 bg-slate-50/50 p-4">
                        ...
                        <div className="flex flex-col gap-1 border-t border-slate-200/50 pt-2 text-xs">
                          <p className="flex items-center gap-2 text-slate-600">
                            <span className="font-bold opacity-50">SĐT:</span> {c.phone || "N/A"}
                          </p>
                          <p className="flex items-center gap-2 text-slate-600">
                      ...
                      <div className="col-span-full py-4 text-center">
                        <p className="text-xs italic text-slate-400">Chưa có thông tin người liên hệ.</p>
                      </div>
                    )}
                  </div>
                </div>
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-slate-500">{m.title || "Chức vụ chưa rõ"}</p>
                            </div>
                          </div>
           ...
           {activeTab === "schedule" && (
              <div className="space-y-4">
                ...
                {d.events?.length ? (
                    ...
                    <div key={event.id} className="group rounded-xl border border-slate-200 bg-white p-6 shadow-sm transition-all hover:border-primary/30">
                      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                        <div className="space-y-2">
                          ...
                          <p className="text-sm font-bold text-slate-900">{event.title || "Chưa có tiêu đề"}</p>
                  ...
                  <EmptyState message="Chưa có lịch trình." />
                )}
              </div>
           )}
              ...
              <div className="space-y-6">
                <section className="space-y-4 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                  ...
                  {d.checklist?.length ? (
                      ...
                      {d.checklist.map((item, index: number) => (
                        <div key={item.id ?? index} className="group rounded-lg border border-slate-100 bg-slate-50 p-4 transition-all hover:bg-slate-100/50">
                          <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                            <div className="flex items-center gap-3">
                               <p className="text-sm font-bold text-slate-900">{item.item_name || item.name || "Chưa có tiêu đề"}</p>
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">
                              {item.status !== undefined && item.status !== null ? `Trạng thái: ${item.status === 2 ? "Hoàn thành" : "Đang làm"}` : ""}
                            </span>
                          </div>
                    ...
                    <p className="text-sm text-slate-400">Chưa có checklist.</p>
                  )}
                </section>
              </div>
           )}
           ...
           {activeTab === "followup" && (
              <div className="space-y-6">
                <section className="space-y-4 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                  <div className="flex items-center justify-between">
                    <h3 className="text-sm font-black uppercase tracking-widest text-slate-900">Kết quả & Theo dõi</h3>
                    <button 
                      onClick={() => {
                  ...
                  {d.outcomes?.length ? (
                    ...
                    <p className="text-sm text-slate-400">Chưa có thông tin theo dõi.</p>
                  )}
                </section>
              </div>
           )}
        ...
        <div className="space-y-6">
           <div className="rounded-xl border border-slate-200 bg-slate-50 p-6">
              ...
              <div className="flex items-center gap-3">
                 ...
                 <div>
                    <p className="text-sm font-bold text-slate-900">{ownerUser?.fullName || d.owner?.full_name || "Admin System"}</p>
                    <p className="text-[10px] font-medium text-slate-500">{d.host_unit?.unit_name || ownerUser?.unit?.unit_name || "Bộ phận Đầu tư"}</p>
                 </div>
              </div>
           </div>
        </div>
        ...
        <DialogContent className="sm:max-w-[500px]">
          ...
          <div className="grid grid-cols-2 gap-4 py-4">
            <div className="col-span-2 space-y-1">
              <label htmlFor="member-fullname" className="text-[10px] font-black uppercase text-slate-400">Họ tên *</label>
              <Input 
                id="member-fullname"
            ...
            <div className="space-y-1">
              <label htmlFor="member-role" className="text-[10px] font-black uppercase text-slate-400">Chức vụ</label>
              <Input 
                id="member-role"
            ...
            <div className="col-span-2 space-y-1">
              <label htmlFor="member-org" className="text-[10px] font-black uppercase text-slate-400">Tổ chức/Doanh nghiệp</label>
              <Input 
                id="member-org"
            ...
            <div className="space-y-1">
              <label htmlFor="member-identity" className="text-[10px] font-black uppercase text-slate-400">CCCD/Passport</label>
              <Input 
                id="member-identity"
      ...
      <Dialog open={modalType?.includes("checklist")} onOpenChange={() => setModalType(null)}>
        <DialogContent className="sm:max-w-[440px]">
          ...
          <div className="space-y-6 py-4">
            ...
            <div className="space-y-1">
              <label htmlFor="edit-checklist-assignee" className="text-[10px] font-black uppercase text-slate-400">Người phụ trách</label>
              <SelectField 
          ...
          <div className="space-y-6 py-4">
            ...
            <div className="space-y-1">
              ...
              <Textarea 
                ...
                rows={4}
                placeholder="Tóm tắt các kết quả quan trọng..."
              />
            </div>
            ...
            <div className="space-y-1">
              ...
              <Textarea 
                ...
                rows={4}
                placeholder="Các công việc cần theo dõi sau đoàn..."
              />
            </div>
          </div>
          <DialogFooter>
      ...
      <ConfirmModal 
        ...
        title="Xác nhận xóa"
        description="Hành động này không thể hoàn tác. Bạn có chắc chắn muốn xóa mục này khỏi danh sách?"
      />
    </div>
  );
}
    ...
    <div className="mb-6 overflow-hidden rounded-2xl border border-primary/20 bg-primary/5 p-1 duration-500 animate-in slide-in-from-top-4">
      <div className="flex flex-col gap-4 p-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-primary">Phê duyệt hồ sơ</h4>
            <p className="text-sm font-bold text-slate-900">Vui lòng xem xét hồ sơ và đưa ra quyết định.</p>
          </div>
        </div>
        ...
        <div className="border-t border-primary/10 bg-white/50 p-6 duration-300 animate-in fade-in slide-in-from-top-2">
          <div className="flex flex-col gap-4">
            ...
            <textarea
              ...
              placeholder="Nhập lý do cụ thể để nhân viên nắm rõ nội dung cần điều chỉnh..."
