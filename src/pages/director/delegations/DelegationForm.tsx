    ...
    <div className="mx-auto max-w-5xl pb-20 duration-500 animate-in slide-in-from-bottom-4">
      ...
      <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-4">
          <button 
            onClick={() => navigate(-1)} 
            className="rounded-2xl border border-slate-200 bg-white p-3 text-slate-400 shadow-sm transition-all hover:border-primary/20 hover:text-primary"
          >
            <ArrowLeft size={20} />
          </button>
          <div>
        ...
        <div className="flex items-center gap-3">
          <button 
            onClick={() => navigate(-1)}
            className="rounded-2xl border border-slate-200 bg-white px-6 py-3 text-[11px] font-black uppercase tracking-wider text-slate-600 transition-all hover:bg-slate-50"
          >
            HỦY BỎ
          </button>
          <button
            onClick={handleSave}
            disabled={isLoading}
            className="flex items-center gap-2 rounded-2xl bg-primary px-8 py-3 text-[11px] font-black uppercase tracking-wider text-white shadow-xl shadow-primary/25 transition-all hover:-translate-y-0.5 hover:bg-primary/95 disabled:opacity-50"
          >
      ...
      <div className="no-scrollbar mb-8 flex gap-2 overflow-x-auto rounded-[28px] bg-slate-100 p-1">
      ...
      {/* Form Content Area */}
      <div className="rounded-[40px] border border-slate-100 bg-white p-6 sm:p-10 shadow-xl shadow-slate-200/40">
        {activeTab === "basic" && (
          <div className="space-y-8 duration-300 animate-in fade-in">
            ...
            <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
              <FormField label="Tên phái đoàn / Dự án" required className="md:col-span-2">
                <input
                  className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-5 py-3.5 text-sm font-bold outline-none transition-all focus:border-primary focus:bg-white focus:ring-4 focus:ring-primary/5"
                  value={formData.name}
              ...
              <div className="grid grid-cols-2 gap-4">
                <FormField label="Loại phái đoàn">
                  <select 
                    className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-5 py-3.5 text-sm font-bold outline-none transition-all focus:border-primary focus:bg-white"
                    value={formData.type}
                    onChange={(e) => setFormData({ ...formData, type: e.target.value })}
                  >
                    <option value="Inbound">Inbound (Vào)</option>
                    <option value="Outbound">Outbound (Ra)</option>
                  </select>
                </FormField>
                <FormField label="Mức ưu tiên">
                  ...
                  >
                    <option value="High">Rất cao</option>
              ...
              <FormField label="Quốc gia / Lãnh thổ">
                <div className="relative">
                  ...
                  <input 
                    className="w-full rounded-2xl border border-slate-200 bg-slate-50 py-3.5 pl-12 pr-5 text-sm font-bold outline-none transition-all focus:border-primary focus:bg-white" 
            ...
            <FormField label="Mục tiêu & Nội dung làm việc">
              <textarea
                rows={4}
                className="w-full rounded-3xl border border-slate-200 bg-slate-50 px-5 py-4 text-sm font-medium leading-relaxed outline-none transition-all focus:border-primary focus:bg-white"
          ...
          <div className="space-y-8 duration-300 animate-in fade-in">
            <div className="flex items-center justify-between">
              ...
              <button 
                ...
                className="flex items-center gap-2 rounded-xl bg-slate-900 px-4 py-2 text-[10px] font-black uppercase tracking-widest text-white transition-all hover:bg-slate-800"
              >
            ...
            <div className="space-y-4">
              {members.map((member, i) => (
                <div key={i} className="group relative rounded-3xl border border-slate-100 bg-slate-50 p-6 transition-all hover:border-primary/20">
                  <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
                    <FormField label="Họ và tên">
                      <input 
                        className="w-full rounded-xl border border-slate-100 bg-white px-3 py-2 text-sm font-bold outline-none focus:border-primary"
                        value={member.name}
                    ...
                    <FormField label="Liên hệ">
                      <div className="flex items-center gap-2">
                        <input 
                          className="flex-1 rounded-xl border border-slate-100 bg-white px-3 py-2 text-sm font-bold outline-none focus:border-primary"
        ...
        {activeTab === "schedule" && (
          <div className="space-y-8 duration-300 animate-in fade-in">
            ...
            <div className="space-y-4">
              {schedule.map((item, i) => (
                <div key={i} className="group relative flex gap-4 rounded-3xl border border-slate-100 bg-slate-50 p-6 transition-all hover:border-primary/20">
        ...
        {activeTab === "files" && (
          <div className="space-y-8 duration-300 animate-in fade-in">
            <div className="flex h-64 flex-col items-center justify-center rounded-[32px] border-2 border-dashed border-slate-200 bg-slate-50 text-slate-400 transition-all hover:bg-slate-100">
...
function TabButton({ active, onClick, icon, label }: any) {
  return (
    <button
      onClick={onClick}
      className={cn(
        "flex items-center gap-2 whitespace-nowrap rounded-2xl px-6 py-3 text-[11px] font-black uppercase tracking-wider transition-all",
